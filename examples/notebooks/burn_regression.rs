...
for epoch in 1..=400 {
    ...
    if epoch % 100 == 0 || epoch == 1 { println!("epoch {epoch:>3}  mse = {mse:.4}"); }
    let grads = GradientsParams::from_grads(loss.backward(), &model);
    model = optimizer.step(0.05, model, grads);
}
println!("final rmse = ${:.4}", mse.sqrt());
println!("forge_metric:rmse={}", mse.sqrt());

// Predict in the same cell: the Flex device/model types are kept local, which
// avoids persisting opaque Burn types across notebook cells.
