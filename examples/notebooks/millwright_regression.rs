...
// total_bill(0), tip(1), sex, smoker, day, time, size(6).
...
for line in tips_csv.lines().skip(1) {
    let c: Vec<&str> = line.split(',').collect();
    let cell = |i: usize| c[i].trim().trim_matches('"').parse::<f64>();
    if let (Ok(bill), Ok(tip), Ok(size)) = (cell(0), cell(1), cell(6)) {
        feats.push(vec![bill, size]);
        target.push(tip);
    }
}
println!("Loaded {} rows (predict tip from total_bill, size)", feats.len());

//# %% train
// Deterministic 80/20 split: every 5th row is held out.
let cols = vec!["total_bill".to_string(), "size".to_string()];
let (mut tr_x, mut tr_y, mut te_x, mut te_y) = (vec![], vec![], vec![], vec![]);
for (i, (row, &y)) in feats.iter().zip(&target).enumerate() {
