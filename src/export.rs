...
pub fn notebook_html(document: &NotebookDocument) -> String {
    let body = document
        ...
        .collect::<String>();
    format!(
        "<!doctype html><meta charset=\"utf-8\"><title>Forge ML notebook</title><style>body{{font:16px system-ui;max-width:960px;margin:40px auto;padding:0 20px}}pre{{white-space:pre-wrap;background:#17191d;color:#eee;padding:16px;border-radius:8px}}.markdown pre{{background:none;color:inherit;padding:0}}</style><main>{body}</main>"
    )
}
...
pub fn dataset_report(name: &str, dataset: &Dataset) -> String {
    ...
    let rows = dataset
        ...
        .collect::<String>();
    html(
        &format!("Dataset report — {}", escape(name)),
        &format!(
            "<h1>{}</h1><p>{} rows × {} columns. Preview limited to 100 rows.</p><h2>Quality alerts</h2><ul>{alerts}</ul><h2>Column profile</h2><table><tr><th>Column</th><th>Missing</th><th>Numeric</th><th>Unique</th><th>Min</th><th>Max</th><th>Mean</th><th>Std dev</th></tr>{profile_rows}</table><h2>Numeric correlations</h2><p>Bounded to {} rows and {} numeric columns; strongest 20 pairs shown.</p><table><tr><th>Left</th><th>Right</th><th>Pearson r</th></tr>{correlations}</table><h2>Preview</h2><div class=\"scroll\"><table><tr>{headers}</tr>{rows}</table></div>",
            escape(name),
            dataset.rows.len(),
            dataset.columns.len(),
            quality.correlation_rows,
...
pub fn experiment_report(runs: &[ExperimentRun], metric: &str) -> String {
    let rows = runs
        ...
        .collect::<String>();
    html(
        &format!("Experiment comparison — {}", escape(metric)),
        &format!(
            "<h1>Experiment comparison</h1><p>Metric: <strong>{}</strong></p><table><tr><th>Run</th><th>Final</th><th>Steps</th><th>Tags</th><th>Git commit</th></tr>{rows}</table><h2>Run manifests</h2><pre>{}</pre>",
            escape(metric),
            escape(&serde_json::to_string_pretty(runs).unwrap_or_default())
        ),
    )
}
...
fn html(title: &str, body: &str) -> String {
    format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width\"><title>{title}</title><style>body{{font:15px system-ui;max-width:1100px;margin:40px auto;padding:0 20px;color:#202124}}table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #ccd1d5;padding:7px;text-align:left}}th{{background:#eef2f5}}.scroll{{overflow:auto}}pre{{white-space:pre-wrap;background:#17191d;color:#eee;padding:16px;border-radius:8px}}</style></head><body>{body}<footer><p>Generated by Forge ML {}</p></footer></body></html>",
...
mod tests {
    ...
    fn training_bundle_contains_attested_portable_artifacts() {
        ...
        legacy_manifest["total_uncompressed_bytes"] = serde_json::json!(
            entries
                .iter()
