...
pub fn extra_builtin_themes() -> Vec<NamedTheme> {
    // A flat color-tuple builder; the palette slots read clearest as positional args.
...
mod tests {
    ...
    fn builtin_themes_are_named_uniquely_and_distinct() {
        ...
        for theme in &themes {
            ...
            assert_eq!(
                serde_json::from_str::<Palette>(&json).unwrap(),
                theme.palette
            );
        }
    }
