...
The training overview accepts framework-neutral `forge_training:` events from Millwright, Burn, or custom Rust training code. It validates finite scores, losses, metrics, and throughput; limits job, parameter, checkpoint, and failure text to 64 KiB; and retains the newest 10,000 events. **Open metric plots** converts available epoch/batch loss, epoch metric, completed-trial score, and batch-throughput events into native structured plots and switches to the Plots pane. Repeating the action refreshes same-name plots, which then participate in durable history and every JSON, SVG, PNG, PDF, and HTML export. **Export JSON** preserves the typed event stream, **Export CSV** writes an event index plus escaped event JSON, and both exports stop at 64 MiB. **Import JSON** reopens an exported stream using a 16 MiB input limit and replaces live telemetry only after every event passes validation. **Clear events** releases the live history. Training events remain memory-only unless explicitly exported or captured in an experiment artifact.
