...
The Plots pane accepts legacy metrics/vectors and versioned `forge_plot:` JSON. Structured plots support lines, scatter, bars, filled areas, histograms, box summaries, heatmaps, ROC, precision–recall, residuals, and feature importance. Each series can be hidden, axes can be transformed to log10, and definitions can be exported as JSON, standalone SVG, a native 960×540 PNG, or self-contained interactive HTML. The HTML viewer provides series toggles, hover coordinates, wheel zoom, drag pan, and view reset without loading a CDN or other network resource. Raster and HTML canvas export run entirely inside Forge and enforce a 16-megapixel memory-safety limit.
