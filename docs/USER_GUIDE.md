...
Remote execution profiles accept HTTPS Jupyter or JupyterHub base URLs; plain HTTP is restricted to localhost development servers. Tokens are stored in the OS credential manager and are passed to bounded requests without persistence in project files or placement in command-line arguments. **Test Jupyter** requests the server's kernelspec list with a 10-second timeout and 1 MiB response limit. Enter a discovered kernelspec name and use **Start kernel** to create one server-side kernel; Forge displays its server-issued ID and enables **Stop remote kernel** for explicit cleanup. The session is memory-only and Forge permits one managed remote kernel at a time. Enter code and choose **Run on remote kernel** to execute through an authenticated TLS WebSocket. Forge correlates messages to its request, collects text streams, tracebacks, and HTML, Markdown, SVG, PNG, or JSON MIME results, and completes only after both execute-reply and idle messages. Rich payloads are shown as inert source in a collapsible view rather than executed as active content. While a request is running, **Interrupt execution** uses a separate control lane so it remains responsive even though the execution channel is blocked. Remote code is limited to 1 MiB, combined returned text/rich output to 2 MiB, and reads to 30 seconds. stdin and direct notebook-cell routing remain future work.
