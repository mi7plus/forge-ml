...
Builds compiled with the optional `millwright` feature can also choose **Tools → Import via Millwright** for CSV and Parquet. Forge uses the published Millwright 2.2.1 crate, performs native loading on the background integration worker, then applies the same row, column, decoded-text, and cell budgets before inserting the result into the Arrow-backed workspace. No local Millwright repository is consulted.
