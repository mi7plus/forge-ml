...
Import CSV, TSV, JSON Lines, Parquet, or Arrow IPC from the Data pane or Tools menu. Parsing runs in the background, shows an activity indicator, and opens a successful import in the adjustable lower-right viewer. Interactive imports are limited to 512 MiB on disk, one million rows, 10,000 columns, 512 MiB of decoded cell text, and 16 MiB per cell or JSON line. Parquet is decoded in bounded batches and JSON Lines is scanned without loading the whole source into one string. Query or stream larger sources. Export tables from their Data menu. Runs can be compared, cloned, archived, and exported as ZIP bundles.
