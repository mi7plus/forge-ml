...
The training overview accepts framework-neutral `forge_training:` events from Millwright, Burn, or custom Rust training code. It validates finite scores, losses, metrics, and throughput; limits job, parameter, checkpoint, and failure text to 64 KiB; and retains the newest 10,000 events. **Export JSON** preserves the typed event stream, **Export CSV** writes an event index plus escaped event JSON, and **Clear events** releases the live history. Training events remain memory-only unless explicitly exported or captured in an experiment artifact.
