...
Remote execution profiles accept HTTPS Jupyter or JupyterHub base URLs; plain HTTP is restricted to localhost development servers. Tokens are stored in the OS credential manager and are passed to the bounded connectivity probe over child-process stdin, never persisted in project files or placed in command-line arguments. **Test Jupyter** requests the server's kernelspec list with a 10-second timeout and 1 MiB response limit. Remote kernel session creation and WebSocket execution are not yet implemented.
